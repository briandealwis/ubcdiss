...
by the \latexpackage{natbib} package); see \autoref{tbl:natbib:cite}.
Note that the form of the citation will depend on the 
bibliography style (described in the next section); this document
has been formatted with a numbered style, but other styles exist.
\begin{table}
...
basis (\latexpackage{bibunits}), as footnotes (\latexpackge{footbib}),
...
in \LaTeX: his suggested manner for typesetting tables produce
abysmally ugly abominations.  These suggestions have unfortunately
been replicated in most \LaTeX\ tutorials.  Fortunately these
abominations are easily avoided by simply ignoring his instructions
on horizontal and vertical rules (specifically the use of \verb+\hline+
and \verb+|+) and using the \latexpackage{booktabs} package instead.

The \latexpackage{booktabs} package provides new rules, producing
tables in the form used by most professionally-edited journals.
...
\item \webref{http://www.ctan.org}{\ac{CTAN}} is \emph{the} comprehensive
    ...
    Should you have a particular need, somebody else is almost
    certainly to have had the same need before you.
...
\item Chemists and those requiring chemical formula should definitely
    checkout the \latexpackage{mhchem} package.
